...
export const TOOLS = [
  ...
  { name: "find_production_matches", category: "tournament_discovery", access: "read", description: "Batch duplicate lookup: pass candidates_json (JSON array string, max 25 items) and get production matches for each in one call. Use to classify search-history findings as New / Duplicate / Enrichment." },
  ...
  { name: "get_roll_forward_candidates_v2", category: "roll_forward", access: "read", description: "Select production source tournaments for target-year roll-forward research with full linked-venue context, target-year roll-forward status, and classified sibling matches. V1 remains unchanged." },
  { name: "get_tournament_roll_forward_context", category: "roll_forward", access: "read", description: "Return one production source tournament's full tournament, linked venue, roll-forward-history, and target-year sibling-match context." },
  ...
  { name: "upsert_roll_forward_log", category: "roll_forward", access: "write", description: "Stage or update a roll-forward research entry on (parent_tournament_id, target_year). Status transitions are validated server-side. Statuses: unresearched → pending / no_dates_announced / ambiguous / ready_to_create / linked_existing / discontinued / done. Writes target-year staging fields and verification flags. Only provided fields are updated; sibling_id is protected against silent replacement. Requires ENABLE_MCP_WRITES=true." },
