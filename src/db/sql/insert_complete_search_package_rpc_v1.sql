...
begin
  ...
  if found then
    ...
    insert into public.tournament_search_runs (
      ...
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_run -> 'states', '[]')) as x),
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_run -> 'sports', '[]')) as x),
      ...
      0, 0, 0, 0, 0, 0, 0,
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_run -> 'organizer_domains', '[]')) as x),
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_run -> 'organizer_names', '[]')) as x),
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_run -> 'venue_names', '[]')) as x),
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_run -> 'high_value_sources', '[]')) as x),
  ...
  loop
    insert into public.tournament_search_organizer_intelligence (
      ...
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_intel -> 'states', '[]')) as x),
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_intel -> 'sports', '[]')) as x),
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_intel -> 'tournament_families', '[]')) as x),
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_intel -> 'venue_clusters', '[]')) as x),
      ARRAY(select x from jsonb_array_elements_text(coalesce(v_intel -> 'monitoring_urls', '[]')) as x),
